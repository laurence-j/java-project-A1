...
public class AnimationPanel extends JComponent implements Runnable {
	...
	protected void createNewShape(int x, int y) {
		...
		case 0:
			...
			shapes.add(new MovingRectangle(					
					...
					marginWidth,
					marginHeight,
					this.getCurrentBorderColor(),
					this.getCurrentFillColor(),
					this.currentPath ));
			break;
		...
		case 4:
			...
			System.out.println("Added QuadCircle!");
			shapes.add(new MovingQuadCircles(
					x,
					y,
					this.getCurrentWidth(),
					this.getCurrentHeight(),
		...
		case 5:
			...
			System.out.println("Added PixelArt!");
			shapes.add(new MovingPixelArt(
					x,
					y,
					this.getCurrentWidth(),
					this.getCurrentHeight(),
	...
	public String getSortedInfo() {
		ArrayList<MovingShape> a = new ArrayList<MovingShape>();
		for (int count = 0; count < shapes.size(); count++) {
			a.add(shapes.get(count));		//Add movingshapes into an arraylist for sorting
		}

		Collections.sort(a);	//Sorting arraylist
		String info = "";		//Creating string for output display
